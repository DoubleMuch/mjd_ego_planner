...
public:
  ...
  inline int getInflateOccupancy_bruno(Eigen::Vector3i id);
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos) {
  ...
  return 0;//int(md_.occupancy_buffer_inflate_[toAddress(id)]);
}
...
inline int GridMap::getInflateOccupancy_bruno(Eigen::Vector3i id) {
  if (id(0) < 0 || id(0) >= mp_.map_voxel_num_(0) || id(1) < 0 || id(1) >= mp_.map_voxel_num_(1) ||
      id(2) < 0 || id(2) >= mp_.map_voxel_num_(2))
    return -1;
