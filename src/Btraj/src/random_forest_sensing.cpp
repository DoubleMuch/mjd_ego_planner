...
{ 
   ...
   int size = 25;
   double x = floor(_x_l/_resolution) * _resolution + _resolution / 2.0;
   double y = floor(_y_l/_resolution) * _resolution + _resolution / 2.0;
   for(int i=0;i<ceil(_x_size/_resolution);++i){
      for(int j=0;j<ceil(_y_size/_resolution);++j){
         if(i/size%2==j/size%2){
            if(i%size&&i%size!=size-1&&j%size&&j%size!=size-1)
               for(int k=0;k<ceil(_z_limit/_resolution);++k){
                  pt_random.x = x + (i) * _resolution;
                  pt_random.y = y + (j) * _resolution;
                  pt_random.z = k * _resolution;
   ...
   // double x = 6,y=6,z=6;
   // int widNum = ceil(2.8/_resolution);

   // for(int r = -widNum/2.0; r < widNum/2.0; r ++ )
   //    for(int s = -widNum/2.0; s < widNum/2.0; s ++ ){  
   //       int heiNum = ceil(z/_resolution);
