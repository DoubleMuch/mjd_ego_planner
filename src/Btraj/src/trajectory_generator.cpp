...
#define _SIZE 0.3
    ...
    int all_val     = ctrlP_num + segment_num*3*2;
    int all_con_num = con_num + ctrlP_num*2;
    double x_var[all_val];
    double primalobj;
    ...
    {   
        ...
        {   
            ...
            {   
                ...
                con_bdk.push_back(make_pair(MSK_BK_RA,make_pair(lo_bound+_SIZE/scale_k,up_bound+_SIZE/2/scale_k)));
                con_bdk.push_back(make_pair(MSK_BK_RA,make_pair(lo_bound-_SIZE/2/scale_k,up_bound-_SIZE/scale_k)));
            }
        } 
    }
    double lo_bound, up_bound;
    for(int k = 0; k < segment_num*3*2; k++){
        lo_bound = 0;//-_SIZE/2/corridor[k/6].t;
        up_bound = _SIZE/2/corridor[k/6].t;
        var_bdk.push_back(make_pair( MSK_BK_RA, make_pair(lo_bound,up_bound)));
    }
    ...
    r = MSK_maketask(env,all_con_num, all_val, &task); 
...
//######################################################################
    ...
    for( i = 0; i < all_con_num && r == MSK_RES_OK; i++ ) {
    ...
    {    
        ...
        for(int k = 0; k < segment_num*6; k ++)
        {
            qsubi[idx] = ctrlP_num + k;   
            qsubj[idx] = ctrlP_num + k;
            qval[idx]  = 100;
            idx++;
        }
    }
    ...
    for(int i = 0; i < ctrlP_num; i++)
        d_var(i) = x_var[i];
    for(int i=ctrlP_num;i<all_val;++i)
        if(x_var[i]*corridor[(i-ctrlP_num)/6].t > _SIZE/2)       ROS_WARN("%lf",x_var[i]*corridor[(i-ctrlP_num)/6].t);
