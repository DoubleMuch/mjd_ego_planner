...
void timeAllocation(vector<Cube> & corridor,vector<Vector3d>& grid_path);
...
void rcvOdometryCallbck(const nav_msgs::Odometry odom)
{
    ...
    if(_start_pt(2)<0.5) _start_pt(2) = 0.5;
...
{     
    ...
    ROS_INFO("[Bruno Node] receive the way-point");
...
vector<pcl::PointXYZ> pointInflate( pcl::PointXYZ pt)
{
    int num   = 0;//int(_cloud_margin * _inv_resolution);
    int num_z = 0;//max(1, num / 2);
...
{       
    ...
    //     return true;
    if(grid_map_->getInflateOccupancy(checkPt))
        return true;
    return false;
}
...
{   
    ...
    { 
        ...
        grid_map_->posToIndex(coord,index);
        vertex_idx.row(i)=index;
    }
    ...
    {   
        ...
        {   
            ...
            {    
                ...
                {
                    //occupy = collision_map->Get( (int64_t)id_x, (int64_t)id_y, (int64_t)id_z).first.occupancy;    
                    occupy = grid_map_->getInflateOccupancy_bruno(Vector3i(id_x,id_y,id_z));
        ...
        for(int i = 0; i < 8; i++)
        {   
            int index_x = vertex_idx(i, 0);
            int index_y = vertex_idx(i, 1);
            int index_z = vertex_idx(i, 2);

            Vector3i index(index_x, index_y, index_z);
            //Vector3d pos = collision_map->GridIndexToLocation(index);

            grid_map_->indexToPos(index,pos);
...
*/       
    ...
    Vector3i pt_index;
    grid_map_->posToIndex(pt,pt_index);
    grid_map_->indexToPos(pt_index,pt);
...
{   
    ...
    //    return;
    if( _has_target == false  || _has_odom == false) 
       return;    
    ...
    {   
        ...
        ***/
        ...
        //vector<GridNodePtr> searchedNodes;
        if (a_star_->AstarSearch(/*(in-out).norm()/10+0.05*/ _resolution, _start_pt, _end_pt))
        {
            gridPath = a_star_->getPath();
        }
        else
        {
        ...
        timeAllocation(corridor,gridPath);
        visCorridor(corridor);
    }
...
{   
    ...
    double _Vel = _MAX_Vel * 0.8;
    double _Acc = _MAX_Acc * 0.8;
...
bool posincorr(Vector3d pos,Cube c){
    Cube p = generateCube(pos);
    return isContains(c,p);
}
void timeAllocation(vector<Cube> & corridor,vector<Vector3d>& grid_path)
{   
    double my_base_time = _resolution*1.732/_MAX_Vel/0.8;
    int in=-1,out=-1;
    ...
    for(int i=0;i<grid_path.size();++i){
        ...
        out = -1;
        for(int j=0;j<corridor.size();++j){
            if(posincorr(grid_path[i],corridor[j])){
                if(in==-1)
                    in = j;
                out = j;
            }
        ...
        for(int j=in;j<=out;++j){
            corridor[j].t += my_base_time/(out-in+1);
        }
    }
    if(corridor[0].t < _MAX_Vel/_MAX_Vel*2)
        corridor[0].t += _MAX_Vel/_MAX_Vel;
    if(corridor[corridor.size()-1].t < _MAX_Vel/_MAX_Vel*2)
    corridor[corridor.size()-1].t += _MAX_Vel/_MAX_Vel;
}

void initialmap(ros::NodeHandle &nh){
    grid_map_.reset(new GridMap);
    grid_map_->initMap(nh);
    a_star_.reset(new AStar);
    a_star_->initGridMap(grid_map_, Eigen::Vector3i(_x_size*_inv_resolution, _y_size*_inv_resolution, _z_size*_inv_resolution));
}
...
int main(int argc, char** argv)
{
    ...
    nh.param("planning/max_inflate",   _max_inflate_iter, 25);
    ...
    exec_timer_   = nh.createTimer(ros::Duration(0.05),execFSMCallback);
    //safety_timer_ = nh.createTimer(ros::Duration(0.05),checkExecTraj);
